...
class Database:
    ...
    def __init__(self):
        try:
            ...
            messagebox.showerror("Erro de Configuração", f"Não foi possível ler as credenciais do banco: {e}")
            sys.exit(1)
    ...
    def _get_connection(self) -> Optional[pyodbc.Connection]:
        ...
        try:
            ...
            try:
                ...
                messagebox.showerror("Erro de Banco de Dados", f"Não foi possível conectar: {e_inner}")
                return None
    ...
    def admin_get_all_users(self) -> List[Tuple]:
        ...
        if not cursor: return []
        try:
            with cursor: return cursor.execute(query).fetchall()
        except pyodbc.Error as e:
    ...
    def admin_get_user_details(self, user_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            with cursor:
                ...
                user_details = {
                    "nome": user_result.Usu_Nome, "email": user_result.Usu_Email or "",
                    "is_active": user_result.Usu_Ativo, "is_admin": user_result.Usu_Is_Admin,
                    "grupos": set()
                }
                group_results = cursor.execute(query_groups, user_id).fetchall()
                for row in group_results: user_details["grupos"].add(row.Gru_Codigo)
                return user_details
        except pyodbc.Error as e:
    ...
    def admin_create_user(self, username: str, email: str, is_admin: bool, is_active: bool, group_ids: List[int]) -> Tuple[bool, str]:
        ...
        try:
            ...
            if "UNIQUE KEY" in str(e) or "Violation of UNIQUE KEY" in str(e): return False, f"Erro: Nome '{username}' já existe."
            return False, f"Erro DB: {e}"
        except Exception as ex:
            conn.rollback()
            logging.error(f"Admin: Erro inesperado ao CRIAR usuário: {ex}")
            return False, f"Erro: {ex}"
        finally: conn.autocommit = True # Restaura autocommit
    ...
    def admin_get_group_details(self, group_id: int) -> Optional[Dict[str, str]]:
        ...
        try:
            with cursor:
                ...
                if result: return {"nome": result.Gru_Nome, "desc": result.Gru_Descricao or ""}
        except pyodbc.Error as e:
    ...
    def admin_create_group(self, nome: str, desc: Optional[str]) -> Tuple[bool, str]:
        ...
        cursor = self._connect()
        if not cursor: return False, "Falha ao conectar."
        try:
            with cursor: cursor.execute(query, (nome, desc)); return True, "Grupo criado."
        except pyodbc.Error as e:
            logging.error(f"Admin: Erro ao CRIAR grupo: {e}")
            if "UNIQUE KEY" in str(e) or "Violation of UNIQUE KEY" in str(e): return False, f"Erro: Nome '{nome}' já existe."
            return False, f"Erro DB: {e}"
    ...
    def admin_update_group(self, group_id: int, nome: str, desc: Optional[str]) -> Tuple[bool, str]:
        ...
        if not cursor: return False, "Falha ao conectar."
        try:
            with cursor: cursor.execute(query, (nome, desc, group_id)); return True, "Grupo atualizado."
        except pyodbc.Error as e:
    ...
    def admin_delete_group(self, group_id: int) -> Tuple[bool, str]:
        ...
        if not cursor: return False, "Falha ao conectar."
        try:
            with cursor: cursor.execute(query, group_id); return True, "Grupo deletado."
        except pyodbc.Error as e:
            logging.error(f"Admin: Erro ao DELETAR grupo: {e}")
            if "REFERENCE constraint" in str(e): return False, "Erro: Grupo em uso por conexões ou permissões."
            return False, f"Erro DB: {e}"
    ...
    def admin_get_connection_details(self, con_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            with cursor:
                ...
                if result: return dict(zip([col[0] for col in result.cursor_description], result))
        except pyodbc.Error as e:
    ...
    def admin_create_connection(self, data: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        if not cursor: return False, "Falha ao conectar."
        try:
            with cursor: cursor.execute(query, params); return True, "Conexão criada."
        except pyodbc.Error as e:
    ...
    def admin_update_connection(self, con_id: int, data: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        if not cursor: return False, "Falha ao conectar."
        try:
            with cursor: cursor.execute(query, params); return True, "Conexão atualizada."
        except pyodbc.Error as e:
    ...
    def admin_delete_connection(self, con_id: int) -> Tuple[bool, str]:
        ...
        try:
            ...
            if "REFERENCE constraint" in str(e): return False, "Erro: Conexão referenciada em outra tabela."
            return False, f"Erro DB: {e}"
        except Exception as ex:
             conn.rollback()
