...
class DIContainer:
    ...
    def register_singleton(
        self, interface: Type[T], implementation: Union[Type[T], T]
    ) -> "DIContainer":
            ...
            for param_name, param in sig.parameters.items():
                ...
                if param_type == inspect.Parameter.empty:
                    ...
                    if param.default != inspect.Parameter.empty:
                        ...
                        raise TypeError(
                            f"Parâmetro {param_name} em {cls.__name__} precisa de type hint"
                        )
                ...
                try:
                    ...
                    if param.default != inspect.Parameter.empty:
                        ...
                        raise ValueError(
                            f"Não foi possível resolver dependência {param_type} para {cls.__name__}.{param_name}"
                        )

            # Cria a instância
            instance = cls(**kwargs)
            logging.debug(
                f"Instância criada: {cls.__name__} com dependências: {list(kwargs.keys())}"
            )
            return instance
    ...
    def wrapper(*args, **kwargs):
        ...
        for param_name, param in sig.parameters.items():
            ...
            try:
                ...
                if param.default == inspect.Parameter.empty:
                    raise ValueError(
                        f"Não foi possível resolver dependência {param_type} para {func.__name__}.{param_name}"
                    )
...
def setup_dependencies():
    ...
    _ = get_container()  # Inicializa o container
