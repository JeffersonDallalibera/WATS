...
class DatabaseConfig(BaseModel):
    ...
    trust_server_certificate: bool = Field(default=True, description="Confiar no certificado do servidor")
    ...
    def validate_connection_info(cls, values):
        ...
        if db_type == 'sqlserver':
            if not conn_string and not host:
                raise ValueError("Para SQL Server é necessário fornecer connection_string OU host/database")
            ...
            if not integrated and not conn_string:
                if not username or not password:
                    raise ValueError("Para SQL Server sem integrated security é necessário username E password")
...
class RecordingConfig(BaseModel):
    ...
    max_file_size_mb: int = Field(default=100, ge=1, le=1000, description="Tamanho máximo do arquivo em MB")
