...
class SimpleFreeRDPWrapper:
    ...
    def _build_freerdp_command(self, data: Dict[str, str]) -> List[str]:
        ...
        cmd.extend([
            ...
            '/drive:shared,/tmp' if sys.platform.startswith('linux') else '/drive:shared,C:\\temp',
            '+home-drive',
            '/network:auto'
        ])
    ...
    def create_rdp_file(self, connection_data: Dict[str, str], 
                       filename: str = None) -> str:
        """Cria arquivo .rdp para usar com outros clientes"""
        if not filename:
            filename = os.path.join(tempfile.gettempdir(), 
                                  f"wats_rdp_{connection_data['ip']}.rdp")
