...
class RDPConnector:
    ...
    def _check_linux_rdp(self) -> Tuple[bool, str]:
        ...
        return False, "Nenhum cliente RDP encontrado. Instale: rdesktop, xfreerdp, remmina ou vinagre"
...
"""
    ...
    def _execute_command(self, cmd: List[str], client_name: str, data: Dict[str, str] = None) -> bool:
        ...
        try:
            ...
            for arg in cmd:
                ...
                if ('/p:' in arg and len(arg) > 3) or \
                   ('-p' in cmd and cmd.index(arg) == cmd.index('-p') + 1) or \
                   (data and arg == data.get('pwd', '')):
                    masked_cmd.append('***')
                else:
