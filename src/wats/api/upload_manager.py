...
class UploadManager:
    ...
    def __init__(self, upload_client: RecordingUploadClient, 
                 max_retries: int = 3, max_concurrent_uploads: int = 2,
    ...
    def _load_state(self):
        ...
        try:
            ...
            logging.info(f"Upload state loaded: {len(self.completed_uploads)} completed, {len(self.failed_uploads)} failed")
    ...
    def cleanup_old_tasks(self, days_to_keep: int = 7):
        ...
        with self.lock:
            ...
            to_remove = [
                task_id for task_id, task in self.completed_uploads.items()
            ...
            to_remove = [
                task_id for task_id, task in self.failed_uploads.items()
                if task.created_at < cutoff_date
            ]
            for task_id in to_remove:
                self.failed_uploads.pop(task_id)
