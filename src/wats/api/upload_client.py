...
from .exceptions import ApiError, AuthenticationError, NetworkError, ServerError, UploadError
...
class RecordingUploadClient:
    ...
    def __init__(self, api_base_url: str, api_token: str, 
        ...
        self.session.headers.update({
            ...
            'User-Agent': 'WATS-RecordingUploader/1.0'
        })
    ...
    def upload_recording(self, video_file: Path, metadata_file: Path, 
        ...
        try:
            ...
            metadata_result = self._upload_file(
                metadata_file, 
                upload_session['metadata_upload_url'],
                progress_callback=lambda sent, total: progress_callback(sent, total_size) if progress_callback else None
            )
            ...
            video_result = self._upload_file(
                ...
                progress_callback=lambda sent, total: progress_callback(metadata_size + sent, total_size) if progress_callback else None
            )
    ...
    def _create_upload_session(self, metadata: Dict[str, Any], file_size: int) -> Dict[str, Any]:
        ...
        response = self.session.post(
            f"{self.api_base_url}/uploads/create-session",
            json=payload,
            timeout=self.timeout
        )
        response.raise_for_status()
    ...
    def _upload_file(self, file_path: Path, upload_url: str, 
                     progress_callback: Optional[Callable[[int, int], None]] = None) -> Dict[str, Any]:
        ...
        with open(file_path, 'rb') as f:
            ...
            response = self.session.post(
                upload_url,
                files=files,
                timeout=self.timeout * 2  # Double timeout for file uploads
            )
            response.raise_for_status()
    ...
    def _finalize_upload(self, session_id: str) -> Dict[str, Any]:
        ...
        response = self.session.post(
            f"{self.api_base_url}/uploads/{session_id}/finalize",
            timeout=self.timeout
        )
        response.raise_for_status()
    ...
    def get_upload_status(self, session_id: str) -> Dict[str, Any]:
        ...
        response = self.session.get(
            f"{self.api_base_url}/uploads/{session_id}/status",
            timeout=self.timeout
        )
        response.raise_for_status()
    ...
    def delete_recording(self, file_id: str) -> bool:
        ...
        try:
            response = self.session.delete(
                f"{self.api_base_url}/recordings/{file_id}",
                timeout=self.timeout
            )
            response.raise_for_status()
            ...
            raise ServerError(f"Failed to delete recording: {e.response.text}", e.response.status_code)
    ...
    def list_recordings(self, limit: int = 50, offset: int = 0) -> Dict[str, Any]:
        ...
        response = self.session.get(
            f"{self.api_base_url}/recordings",
            params=params,
            timeout=self.timeout
        )
        response.raise_for_status()
