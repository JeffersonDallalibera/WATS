...
class FileRotationManager:
    ...
    def __init__(
        ...
        logging.info(
            f"FileRotationManager initialized - Dir: {self.recordings_dir}, "
    ...
    def start_automatic_cleanup(self):
        ...
        self.cleanup_thread = threading.Thread(
            target=self._cleanup_loop, daemon=True, name="FileRotationManager-Cleanup"
        )
    ...
    def get_storage_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "size_usage_percent": (
                    (total_size / self.max_total_size) * 100 if self.max_total_size > 0 else 0
                ),
    ...
    def force_cleanup_by_age(self, max_age_days: int) -> Dict[str, Any]:
        ...
        try:
            ...
            logging.info(
                f"Force cleanup by age ({max_age_days} days): "
                f"deleted {files_deleted} files, freed {space_freed / (1024*1024):.1f} MB"
            )

            return {"files_deleted": files_deleted, "space_freed_mb": space_freed / (1024 * 1024)}
    ...
    def force_cleanup_by_size(self, target_size_gb: float) -> Dict[str, Any]:
        ...
        try:
            ...
            logging.info(
                f"Force cleanup by size ({target_size_gb}GB): "
                f"deleted {files_deleted} files, freed {space_freed / (1024*1024):.1f} MB"
            )
