...
def demo_config_integration():
    ...
    print(f"   settings overrides: Yes")
    print()
    
    print("🔧 Final Smart Recording Configuration:")
    key_configs = [
        'fps', 'quality', 'inactivity_timeout_minutes', 
        'pause_on_minimized', 'pause_on_covered', 'debug_window_tracking'
    ]
    
    for key in key_configs:
        value = config.get(key, 'Not set')
        source = "config.json" if key in recording_config_dict.get('smart_recording', {}) else "defaults/settings"
        ...
        try:
            for i in range(30):  # Monitor for 30 seconds
                ...
                if status.get('is_recording'):
                    ...
                    print(f"[{i+1:2d}s] State: {state} | Duration: {duration:.1f}s | Segments: {segments}")
                    ...
                    if activity_info:
                        ...
                        print(f"      Activity: {'Active' if is_active else 'Inactive'} "
