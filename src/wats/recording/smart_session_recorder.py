...
from .window_tracker import WindowTracker, WindowState, WindowInfo
from .interactivity_monitor import InteractivityMonitor, ActivityEvent
...
class SmartSessionRecorder:
    ...
    def __init__(self, output_dir: str, connection_info: Dict[str, Any], 
        ...
        logging.info(f"🎥 SMART SESSION RECORDER INICIALIZADO:")
        ...
        logging.info(f"   🌐 Servidor: {connection_info.get('server_ip', 'N/A')}:{connection_info.get('server_port', 'N/A')}")
        
        # Verifica se diretório é acessível
        if self.output_dir.exists() and self.output_dir.is_dir():
            logging.info(f"   ✅ Diretório existe e é acessível")
        else:
            logging.warning(f"   ⚠️  Diretório pode não estar acessível")
    ...
    def stop_recording(self) -> List[str]:
        ...
        try:
            ...
            if file_paths:
                ...
                logging.info(f"🎬 GRAVAÇÃO FINALIZADA COM SUCESSO:")
                ...
                logging.info(f"   📄 Arquivos criados:")
                ...
                logging.warning(f"⚠️  GRAVAÇÃO FINALIZADA SEM ARQUIVOS CRIADOS")
    ...
    def get_recording_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'window_info': self.window_tracker.get_current_window_info().__dict__ if self.window_tracker else None,
            'activity_info': self.interactivity_monitor.get_activity_summary() if self.interactivity_monitor else None
        }
    ...
    def _capture_and_write_frame(self, sct_instance):
        ...
        try:
            ...
            logging.debug(f"✍️ Frame escrito no arquivo")
            ...
            if self.current_segment:
                ...
                if self.current_segment.frame_count % 300 == 0:
                    ...
                    logging.info(f"📊 GRAVAÇÃO EM PROGRESSO:")
    ...
    def _create_new_segment(self, reason: str) -> bool:
        ...
        try:
            ...
            logging.info(f"🎥 CRIANDO NOVO SEGMENTO DE GRAVAÇÃO:")
            logging.info(f"   📁 Diretório: {self.output_dir}")
            ...
            for fourcc in recommended_codecs:
                try:
                    ...
                    writer = cv2.VideoWriter(
                        str(file_path), fourcc, self.fps, (width, height)
                    )
            ...
            if not self.current_writer or not self.current_writer.isOpened():
                logging.error(f"❌ Failed to create video writer for {file_path} - nenhum codec funcionou")
                return False
    ...
    def _finalize_current_segment(self, reason: str):
        ...
        try:
            ...
            logging.info(f"Finalized segment: {self.current_segment.file_path.name} "
    ...
    def _resume_recording(self, reason: str):
        ...
        if self.state not in [RecordingState.PAUSED, RecordingState.WAITING_FOR_WINDOW, 
        ...
        if not self.window_tracker:
            logging.error(f"   ❌ Window tracker não disponível")
            return
        ...
        if not window_suitable:
            logging.warning(f"   ⚠️  Janela não está adequada, mudando para WAITING_FOR_WINDOW")
            self.state = RecordingState.WAITING_FOR_WINDOW
            return
        
        # Tenta criar novo segmento
        logging.info(f"   📄 Criando novo segmento: create_new_after_pause={self.create_new_file_after_pause}")
        
        if self.create_new_file_after_pause or not self.current_segment:
            if not self._create_new_segment(f"resumed_{reason}"):
                logging.error(f"   ❌ Falha ao criar novo segmento")
                return
            else:
                logging.info(f"   ✅ Novo segmento criado com sucesso")
