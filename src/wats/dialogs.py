...
class ClientSelectorDialog(ctk.CTkToplevel):
    ...
    def __init__(
        self, parent, clients_data: List[Tuple[str, str]], title: str = "Selecione o Cliente"
    ):
    ...
    def _create_widgets(self):
        header = ctk.CTkLabel(
            self, text="📋 Múltiplos Clientes Disponíveis", font=("Segoe UI", 16, "bold")
        )
        header.grid(row=0, column=0, padx=20, pady=(20, 10), sticky="w")

        info = ctk.CTkLabel(
            self, text="Selecione o cliente desejado para acessar a wiki:", font=("Segoe UI", 11)
        )
