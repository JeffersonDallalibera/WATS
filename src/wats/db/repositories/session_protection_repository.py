...
class SessionProtectionRepository(BaseRepository):
    ...
    def _create_protection_direct(
        ...
        try:
            logging.info(
                f"[DB_PROTECTION] Iniciando criação direta de proteção - con_codigo={con_codigo}, user={user_name}"
            )
            logging.debug(
                f"[DB_PROTECTION] Parâmetros: machine={machine_name}, duration={duration_minutes}min, ip={ip_address}"
            )
            logging.debug(
                f"[DB_PROTECTION] Hash da senha (primeiros 10 chars): {password_hash[:10]}..."
            )
            ...
            if table_exists == 0:
                logging.warning(
                    "[DB_PROTECTION] Tabela Sessao_Protecao_WTS não encontrada - criando..."
                )
            ...
            if result and result[0]:
                ...
                logging.info("[DB_PROTECTION] ✅ Proteção criada com sucesso!")
                ...
                cursor.execute(
                    "SELECT COUNT(*) FROM Sessao_Protecao_WTS WHERE Prot_Id = ?", (protection_id,)
                )
                verify_count = cursor.fetchone()[0]
                logging.info(
                    f"[DB_PROTECTION] Verificação pós-inserção: registro encontrado={verify_count > 0}"
                )
            ...
            logging.error("[DB_PROTECTION] Detalhes dos parâmetros que falharam:")
    ...
    def _create_protection_table(self, cursor):
        ...
        try:
            ...
            if final_check > 0:
                logging.info(
                    "[DB_PROTECTION] ✅ Verificação final: Tabela Sessao_Protecao_WTS criada e verificada"
                )
    ...
    def _validate_password_direct(
        self, cursor, con_codigo, password_hash, requesting_user, requesting_machine, ip_address
    ):
        """Valida senha diretamente via SELECT se SP não existir."""
        try:
            logging.info("[DB_PROTECTION] 🔍 VALIDAÇÃO DIRETA DE SENHA")
            ...
            if not result:
                logging.warning("[DB_PROTECTION] ⚠️ Nenhuma proteção ativa encontrada")
            ...
            logging.info("[DB_PROTECTION] Proteção encontrada:")
            ...
            # Compara hashes
            logging.info("[DB_PROTECTION] Comparando hashes:")
            logging.info(f"[DB_PROTECTION]   - Hash fornecido: {password_hash[:16]}...")
            logging.info(f"[DB_PROTECTION]   - Hash armazenado: {stored_hash[:16]}...")
            ...
            if password_hash == stored_hash:
                logging.info("[DB_PROTECTION] ✅ SENHA CORRETA!")
                logging.info(
                    f"[DB_PROTECTION] Usuário {requesting_user} autorizado para servidor {con_codigo}"
                )

                # Registra tentativa bem-sucedida
                logging.info("[DB_PROTECTION] Registrando tentativa bem-sucedida...")
                ...
                logging.info("[DB_PROTECTION] 📝 AUDITORIA: Acesso direto autorizado")
                ...
                logging.warning("[DB_PROTECTION] ❌ SENHA INCORRETA!")
                logging.warning(
                    f"[DB_PROTECTION] Tentativa de acesso negada para {requesting_user}"
                )

                # Registra tentativa malsucedida
                logging.warning("[DB_PROTECTION] Registrando tentativa malsucedida...")
                self._log_access_attempt(
                    cursor,
                    prot_id,
                    con_codigo,
                ...
                logging.warning(
                    "[DB_PROTECTION] 🚨 AUDITORIA: Senha incorreta na validação direta"
                )
                logging.warning(
                    f"[DB_PROTECTION] Conexão {con_codigo} protegida por {protected_by}"
                )
    ...
    def _log_access_attempt(
        ...
        try:
            logging.info("[DB_PROTECTION] 📝 REGISTRANDO TENTATIVA DE ACESSO")
            logging.info("[DB_PROTECTION] Dados do log:")
            ...
            try:
                logging.info(
                    "[DB_PROTECTION] Tentando stored procedure sp_Log_Tentativa_Protecao..."
                )
                cursor.callproc(
                    "sp_Log_Tentativa_Protecao",
                    (prot_id, con_codigo, user, machine_name, ip_address, success),
                )
                logging.info(
                    "[DB_PROTECTION] ✅ Tentativa de acesso registrada via stored procedure"
                )
                return True
            except Exception as sp_error:
                logging.warning(f"[DB_PROTECTION] ⚠️ Falha na stored procedure de log: {sp_error}")
                logging.info("[DB_PROTECTION] Usando fallback - INSERT direto...")
                ...
                if log_table_exists == 0:
                    logging.info("[DB_PROTECTION] Criando tabela Log_Tentativa_Protecao_WTS...")
                    cursor.execute(
                        """
                    ...
                    logging.info("[DB_PROTECTION] ✅ Tabela de log criada")

                # Insere registro de log
                logging.info("[DB_PROTECTION] Inserindo registro na tabela de log...")
                cursor.execute(
                    """
                ...
                logging.info("[DB_PROTECTION] ✅ Tentativa de acesso registrada via INSERT direto")
                logging.info(
                    f"[DB_PROTECTION] Log salvo: Prot_Id={prot_id}, User={user}, Result={result}"
                )
                return True
    ...
    def create_session_protection(
        ...
        """
        logging.info("[DB_PROTECTION] 🔐 INICIANDO CRIAÇÃO DE PROTEÇÃO DE SESSÃO")
        logging.info("[DB_PROTECTION] Parâmetros de entrada:")
        ...
        logging.info(
            f"[DB_PROTECTION]   - password length: {len(password) if password else 0} chars"
        )
        ...
        try:
            ...
            with self.db.get_cursor() as cursor:
                ...
                if sp_exists == 0:
                    logging.warning(
                        "[DB_PROTECTION] ⚠️ Stored procedure não encontrada - usando método direto"
                    )
                    # Usa INSERT direto se a SP não existir
                    result = self._create_protection_direct(
                        cursor,
                        con_codigo,
                        user_name,
                        machine_name,
                    ...
                    logging.info(
                        f"[DB_PROTECTION] Resultado método direto: success={result[0]}, msg='{result[1]}', id={result[2]}"
                    )
                    return result

                # Chama stored procedure para criar proteção
                logging.info(
                    "[DB_PROTECTION] Executando stored procedure sp_Criar_Protecao_Sessao..."
                )
                logging.debug("[DB_PROTECTION] Parâmetros da SP:")
                ...
                if result and result[0]:
                    ...
                    logging.info("[DB_PROTECTION] ✅ PROTEÇÃO CRIADA COM SUCESSO VIA SP!")
                    logging.info(f"[DB_PROTECTION] ID da proteção: {protection_id}")
                    ...
                    if verification:
                        logging.info("[DB_PROTECTION] ✅ Verificação no banco:")
                        ...
                        logging.info(
                            f"[DB_PROTECTION]   - Hash no banco: {verification[6][:16]}..."
                        )
                        logging.info(
                            f"[DB_PROTECTION]   - Hash confere: {verification[6] == password_hash}"
                        )
                    else:
                        logging.error(
                            "[DB_PROTECTION] ❌ Verificação falhou - registro não encontrado no banco!"
                        )

                    # Invalida cache após criação
                    self._invalidate_protection_caches()
                    return True, "Proteção de sessão ativada com sucesso", protection_id
                else:
    ...
    def validate_session_password(
        ...
        """
        logging.info("[DB_PROTECTION] 🔐 VALIDANDO SENHA DE PROTEÇÃO")
        logging.info("[DB_PROTECTION] Parâmetros de validação:")
        logging.info(f"[DB_PROTECTION]   - con_codigo: {con_codigo}")
        ...
        try:
            ...
            with self.db.get_cursor() as cursor:
                ...
                # Verifica se a stored procedure existe
                logging.info(
                    "[DB_PROTECTION] Verificando existência da stored procedure sp_Validar_Protecao_Sessao..."
                )
                cursor.execute(
                    """
                    SELECT COUNT(*) FROM sys.objects
                ...
                sp_exists = cursor.fetchone()[0]
                logging.info(
                    f"[DB_PROTECTION] SP sp_Validar_Protecao_Sessao existe: {sp_exists > 0}"
                )

                if sp_exists == 0:
                    logging.warning(
                        "[DB_PROTECTION] ⚠️ SP de validação não encontrada - usando validação direta"
                    )
                    result = self._validate_password_direct(
                        cursor,
                        con_codigo,
                        password_hash,
                        requesting_user,
                ...
                logging.info(
                    "[DB_PROTECTION] Executando stored procedure sp_Validar_Protecao_Sessao..."
                )
                logging.debug("[DB_PROTECTION] Parâmetros da SP:")
                logging.debug(f"[DB_PROTECTION]   @Con_Codigo = {con_codigo}")
                logging.debug(f"[DB_PROTECTION]   @Prot_Senha_Hash = {password_hash[:16]}...")
                ...
                if result:
                    ...
                    logging.info("[DB_PROTECTION] Dados retornados:")
                    logging.info(f"[DB_PROTECTION]   - Resultado: {resultado}")
                    logging.info(f"[DB_PROTECTION]   - Protection ID: {protection_id}")
                    logging.info(f"[DB_PROTECTION]   - Protegido por: {protected_by}")

                    if resultado == "SUCESSO":
                        logging.info("[DB_PROTECTION] ✅ ACESSO AUTORIZADO!")
                        logging.info(
                            f"[DB_PROTECTION] Usuário {requesting_user} autorizado para servidor {con_codigo}"
                        )
                        logging.info("[DB_PROTECTION] 📝 AUDITORIA: Acesso bem-sucedido")
                        logging.info(
                            f"[DB_PROTECTION] Proteção ID {protection_id} criada por {protected_by}"
                        )
                        ...
                        logging.warning(
                            f"[DB_PROTECTION] 🚨 AUDITORIA: Senha incorreta para servidor {con_codigo}"
                        )
                        ...
                        logging.warning(
                            "[DB_PROTECTION] ⚠️ ACESSO NEGADO - Servidor não protegido")
                        logging.info(
                            f"[DB_PROTECTION] Servidor {con_codigo} não possui proteção ativa"
                        )
                        ...
                        logging.error(
                            f"[DB_PROTECTION] Protection ID: {protection_id}, Protetor: {protected_by}"
                        )
                    ...
                    logging.error("[DB_PROTECTION] ❌ SP retornou resultado nulo")
    ...
    def remove_session_protection(self, con_codigo: int, removing_user: str) -> Tuple[bool, str]:
        """
        ...
        Returns:
            (success, message)
        """
        logging.info("[DB_PROTECTION] 🗑️ REMOVENDO PROTEÇÃO DE SESSÃO")
        logging.info(f"[DB_PROTECTION] Conexão: {con_codigo}")
        ...
        try:
            with self.db.get_cursor() as cursor:
                ...
                logging.info(
                    f"[DB_PROTECTION] Verificando proteção ativa para conexão {con_codigo}..."
                )
                cursor.execute(
                    """
                ...
                if not protection:
                    logging.warning(
                        f"[DB_PROTECTION] ⚠️ Nenhuma proteção ativa encontrada para conexão {con_codigo}"
                    )
                ...
                # Verifica se a stored procedure existe
                logging.info(
                    "[DB_PROTECTION] Verificando stored procedure sp_Remover_Protecao_Sessao..."
                )
                cursor.execute(
                    """
                    SELECT COUNT(*) FROM sys.objects
                ...
                sp_exists = cursor.fetchone()[0]
                logging.info(
                    f"[DB_PROTECTION] SP sp_Remover_Protecao_Sessao existe: {sp_exists > 0}"
                )

                if sp_exists > 0:
                    # Chama stored procedure para remover
                    logging.info(
                        "[DB_PROTECTION] Executando stored procedure sp_Remover_Protecao_Sessao..."
                    )
                    logging.debug("[DB_PROTECTION] Parâmetros da SP:")
                    logging.debug(f"[DB_PROTECTION]   @Con_Codigo = {con_codigo}")
                    ...
                    if result and result[0]:
                        logging.info("[DB_PROTECTION] ✅ PROTEÇÃO REMOVIDA COM SUCESSO VIA SP!")
                        logging.info(
                            f"[DB_PROTECTION] Servidor {con_codigo} desprotegido por {removing_user}"
                        )
                        logging.info("[DB_PROTECTION] 📝 AUDITORIA: Proteção removida via SP")
                        self._invalidate_protection_caches()
                        return True, "Proteção de sessão removida com sucesso"
                    else:
                        logging.warning("[DB_PROTECTION] ❌ SP retornou falha na remoção")
                        return (
                            False, "Não foi possível remover a proteção. Verifique se você é o criador da proteção.", )
                else:
                    # Fallback: UPDATE direto
                    logging.warning("[DB_PROTECTION] ⚠️ SP não encontrada - usando UPDATE direto")
                    logging.info("[DB_PROTECTION] Executando UPDATE direto na tabela...")
                    ...
                    if rows_affected > 0:
                        logging.info(
                            "[DB_PROTECTION] ✅ PROTEÇÃO REMOVIDA COM SUCESSO VIA UPDATE!"
                        )
                        logging.info(
                            f"[DB_PROTECTION] Servidor {con_codigo} desprotegido por {removing_user}"
                        )
                        logging.info(
                            "[DB_PROTECTION] 📝 AUDITORIA: Proteção removida via UPDATE direto"
                        )
                        self._invalidate_protection_caches()
                        return True, "Proteção de sessão removida com sucesso"
                    else:
                        logging.error("[DB_PROTECTION] ❌ Nenhum registro foi atualizado")
    ...
    def remove_user_protections(self, user_name: str):
        ...
        try:
            logging.info(
                f"[DB_PROTECTION] 🔒 LOGOUT: Removendo todas as proteções do usuário {user_name}"
            )
            ...
            with self.db.get_cursor() as cursor:
                ...
                if not user_protections:
                    logging.info(
                        f"[DB_PROTECTION] 🔒 LOGOUT: Nenhuma proteção ativa encontrada para {user_name}"
                    )
                    return True, "Nenhuma proteção ativa encontrada"

                logging.info(
                    f"[DB_PROTECTION] 🔒 LOGOUT: Encontradas {len(user_protections)} proteções para remover"
                )
                ...
                for prot_id, con_codigo, con_nome in user_protections:
                    try:
                        logging.info(
                            f"[DB_PROTECTION] 🔒 LOGOUT: Removendo proteção ID {prot_id} da conexão {con_codigo} ({con_nome})"
                        )
                        ...
                        removed_count += 1
                        logging.info(
                            f"[DB_PROTECTION] ✅ LOGOUT: Proteção ID {prot_id} removida com sucesso"
                        )

                    except Exception as e:
                        logging.error(
                            f"[DB_PROTECTION] ❌ LOGOUT: Erro ao remover proteção ID {prot_id}: {e}"
                        )

                if removed_count > 0:
                    logging.info(
                        f"[DB_PROTECTION] 🔒 LOGOUT CONCLUÍDO: {removed_count} proteções do usuário {user_name} removidas"
                    )
                    self._invalidate_protection_caches()
                    return True, f"{removed_count} proteções removidas com sucesso"
                else:
                    logging.warning(
                        f"[DB_PROTECTION] ⚠️ LOGOUT: Falha ao remover proteções do usuário {user_name}"
                    )
    ...
    @cached(namespace="session_protection", ttl=30)
    def get_user_protected_sessions(self, user_name: str) -> List[Dict[str, Any]]:
        """Retorna lista de sessões protegidas pelo usuário (cache 30s)."""
        try:
            with self.db.get_cursor() as cursor:
                if not cursor:
                    raise DatabaseConnectionError("Falha ao obter cursor.")
                ...
                query = f"""
                    ...
                    FROM Sessao_Protecao_WTS sp
                    INNER JOIN Conexao_WTS c ON sp.Con_Codigo = c.Con_Codigo
                    WHERE sp.Usu_Nome_Protetor = {self.db.PARAM}
                      AND sp.Prot_Status = 'ATIVA'
                      AND sp.Prot_Data_Expiracao > GETDATE()
                    ORDER BY sp.Prot_Data_Criacao DESC
                """
    ...
    @cached(namespace="session_protection", ttl=60)
    def get_protection_statistics(self) -> Dict[str, Any]:
        """Retorna estatísticas do sistema de proteção (cache 60s)."""
        try:
            with self.db.get_cursor() as cursor:
                if not cursor:
                    raise DatabaseConnectionError("Falha ao obter cursor.")
    ...
    def cleanup_orphaned_protections(self) -> Tuple[bool, str, int]:
        ...
        try:
            with self.db.get_cursor() as cursor:
                ...
                count_removed = cursor.rowcount if cursor.rowcount > 0 else 0
                ...
                return (
                    True,
                    f"Limpeza concluída: {count_removed} proteções removidas",
                    count_removed,
                )
            ...
            logging.error(f"Erro inesperada na limpeza: {e}")
            return False, f"Erro inesperado: {e}", 0

    def _invalidate_protection_caches(self):
        """Invalida todos os caches relacionados a proteções de sessão."""
        invalidate_cache(namespace="session_protection")
