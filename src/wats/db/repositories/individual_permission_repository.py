...
class IndividualPermissionRepository(BaseRepository):
    ...
    def grant_individual_access(self, user_id: int, connection_id: int, granted_by_user_id: int, 
        ...
        try:
            with self.db.get_cursor() as cursor:
                ...
                params = (user_id, connection_id, start_date, end_date, granted_by_user_id, datetime.now(), True, observations)
            ...
            with self.db.get_cursor() as cursor:
                ...
                for result in results:
                    if hasattr(result, "cursor_description"):  # pyodbc
                        permission = dict(zip([col[0] for col in result.cursor_description], result))
                        ...
                        cols = ['Id', 'Con_Codigo', 'Con_Nome', 'Con_IP', 'Data_Inicio', 'Data_Fim', 
                               'Ativo', 'Observacoes', 'Data_Criacao', 'Criado_Por']
            ...
            with self.db.get_cursor() as cursor:
                ...
                for result in results:
                    if hasattr(result, "cursor_description"):  # pyodbc
                        ...
                        # Fallback para tupla simples
                        cols = ['Id', 'Usu_Id', 'Usu_Nome', 'Data_Inicio', 'Data_Fim', 
                               'Ativo', 'Observacoes', 'Data_Criacao', 'Criado_Por']
                        permission = dict(zip(cols, result))
    ...
    def grant_temporary_access(self, user_id: int, connection_id: int, granted_by_user_id: int,
                             duration_hours: float, observations: str = None) -> Tuple[bool, str]:
        """
        ...
        # Verificar se já existe uma permissão ativa (permanente ou temporária) para este usuário/conexão
        existing_query = f"""
            ...
            with self.db.get_cursor() as cursor:
                ...
                if existing:
                    ...
                    if existing_end is None:
                        ...
                        return False, f"Usuário já possui acesso temporário ativo até {existing_end.strftime('%d/%m/%Y %H:%M')}."
                ...
                duration_text = f"{duration_hours}h" if duration_hours >= 1 else f"{int(duration_hours * 60)}min"
                final_observations = f"Acesso temporário ({duration_text}). {observations or ''}".strip()
                
                params = (user_id, connection_id, start_date, end_date, granted_by_user_id, datetime.now(), True, final_observations)
                cursor.execute(insert_query, params)
                
                return True, f"Acesso temporário concedido até {end_date.strftime('%d/%m/%Y %H:%M')}."
            ...
            with self.db.get_cursor() as cursor:
                ...
                for result in results:
                    if hasattr(result, "cursor_description"):  # pyodbc
                        ...
                        # Fallback para tupla simples
                        cols = ['Id', 'Usu_Id', 'Usu_Nome', 'Con_Codigo', 'Con_Nome', 'Con_IP', 
    ...
    def list_all_individual_permissions(self, user_id: Optional[int] = None) -> List[Dict[str, Any]]:
        """
