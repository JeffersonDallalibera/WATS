...
class TemporaryAccessDialog(ctk.CTkToplevel):
    ...
    def _create_grant_section(self, parent):
        ...
        ctk.CTkLabel(
            grant_frame, 
            text="✨ Conceder Acesso Temporário", 
        ...
        self.entry_filter_users = ctk.CTkEntry(
            users_frame, placeholder_text="🔍 Buscar usuário..."
        )
        ...
        scrollbar_users = ttk.Scrollbar(users_list_frame, orient="vertical", command=self.listbox_users.yview)
        ...
        scrollbar_conn = ttk.Scrollbar(conn_list_frame, orient="vertical", command=self.tree_connections.yview)
        ...
        self.entry_observations = ctk.CTkEntry(
            duration_frame, placeholder_text="Motivo do acesso temporário...",
            height=35
        )
        ...
        self.lbl_selected_user = ctk.CTkLabel(
            duration_frame, text="Nenhum usuário selecionado", 
        ...
        self.btn_grant_temporary = ctk.CTkButton(
            duration_frame, text="🕐 CONCEDER ACESSO",
    ...
    def _create_monitoring_section(self, parent):
        ...
        ctk.CTkLabel(
            title_frame, 
            text="📋 Permissões Temporárias Ativas", 
            font=("Segoe UI", 16, "bold")
        ).grid(row=0, column=0, sticky="w", padx=10)
        
        # Botão atualizar
        self.btn_refresh = ctk.CTkButton(
            title_frame, text="🔄 Atualizar",
        ...
        scrollbar_active = ttk.Scrollbar(active_frame, orient="vertical", command=self.tree_active.yview)
    ...
    def _create_action_buttons(self):
        ...
        self.btn_revoke_selected = ctk.CTkButton(
            btn_frame, text="❌ Revogar Selecionado",
        ...
        self.btn_cleanup = ctk.CTkButton(
            btn_frame, text="🧹 Limpar Expirados",
    ...
    def _load_connections_list(self):
        ...
        try:
            ...
            for conn in connections:
                if len(conn) >= 5:
                    conn_id, conn_name, conn_ip, group_name, conn_type = conn[0], conn[1], conn[2], conn[3], conn[4]
    ...
    def _filter_connections(self, event=None):
        ...
        try:
            ...
            for conn in self.all_connections:
                ...
                if (search_text in conn_name.lower() or 
                    search_text in conn_ip.lower() or 
                    search_text in conn_group.lower() or
                    search_text in str(conn['id'])):
                    
                    self.tree_connections.insert(
                        "", "end",
                        text=conn_name,
                        values=(conn_ip, conn_group)
                    )
    ...
    def _grant_temporary_access(self):
        ...
        try:
            ...
            if messagebox.askyesno(
                ...
                f"Deseja conceder acesso temporário?\n\n"
    ...
    def _revoke_selected_access(self):
        ...
        try:
            ...
            if not tags or not tags[0]:
                messagebox.showerror("Erro", "Não foi possível identificar a permissão selecionada!")
                return
            ...
            if messagebox.askyesno(
                "Confirmar Revogação", 
                f"Deseja revogar o acesso temporário?\n\n"
                f"Usuário: {usuario}\n"
                f"Conexão: {conexao}\n"
