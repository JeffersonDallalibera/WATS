...
class TestAutoProtectionRemoval(unittest.TestCase):
    ...
    def test_trigger_removes_protection_on_disconnect(self):
        ...
        # Simula dados de uma proteção ativa (dados de exemplo para documentação)
        # protection_data = {
        #     "protection_id": 123,
        #     "connection_id": 456,
    ...
    def test_cleanup_orphaned_protections_repository(self):
        ...
        self.assertGreater(
            len(update_queries), 0, "Deve executar query UPDATE para remover proteções"
        )
    ...
    def test_cleanup_orphaned_protections_manager(self):
        ...
        self.session_repo.cleanup_orphaned_protections = Mock(
            return_value=(True, "Limpeza concluída", 3)
        )
    ...
    def test_cleanup_orphaned_protections_fallback_local(self):
        ...
        self.assertNotIn(
            456, self.manager.protected_sessions, "Proteção antiga deve ter sido removida"
        )
            ...
            try:
                if mock_app.session_protection_manager:
                    success, message, count = (
                        mock_app.session_protection_manager.cleanup_orphaned_protections()
                    )
                    if count > 0:
    ...
    def test_database_trigger_simulation(self):
        ...
        user_connection = {"con_codigo": 456, "usu_nome": "user_test", "connected": True}
