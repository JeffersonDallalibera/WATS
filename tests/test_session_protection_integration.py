...
from docs.session_protection import SessionProtectionManager, configure_session_protection_with_db
import logging
...
class TestSessionProtectionIntegration(unittest.TestCase):
    ...
    def test_create_protection_uses_server(self):
        ...
        result = self.manager.activate_session_protection(
            connection_id=456, password="senha123", protection_data=protection_data
        )
    ...
    def test_validate_password_uses_server(self):
        ...
        # Valida senha
        with patch("socket.gethostname", return_value="PC-TEST"), patch(
            "socket.gethostbyname", return_value="192.168.1.100"
        ):

            result = self.manager.validate_session_password(
                connection_id=456, password="senha123", requesting_user="user_solicitante"
            )
    ...
    def test_remove_protection_uses_server(self):
        ...
        # Configura mock do repositório
        self.mock_session_repo.remove_session_protection.return_value = (
            True,
            "Removido com sucesso",
        )

        # Remove proteção
        result = self.manager.remove_session_protection(connection_id=456, user="user_test")
        ...
        self.mock_session_repo.remove_session_protection.assert_called_once_with(
            con_codigo=456, removing_user="user_test"
        )
    ...
    def test_logging_integration(self):
        ...
        self.assertTrue(
            any("ACESSO AUTORIZADO VIA SERVIDOR" in record for record in log_capture.output)
        )
