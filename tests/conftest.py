...
def mock_settings(temp_dir: Path) -> Settings:
    ...
    config_data = {
        "database": {"type": "sqlite", "connection_string": f"sqlite:///{temp_dir}/test.db"},
        "recording": {"enabled": False, "output_path": str(temp_dir / "recordings")},
        "api": {"enabled": False, "base_url": "http://localhost:8000"},
    }
...
class TestHelper:
    ...
    def assert_log_contains(caplog, level: str, message: str) -> None:
        ...
        assert any(
            message in msg for msg in messages
        ), f"Log message '{message}' not found in {level} logs"
